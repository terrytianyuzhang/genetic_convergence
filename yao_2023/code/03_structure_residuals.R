...
residual_list <- readRDS(file = glue(work_directory, "data/intermediate_data/poisson_residual.rds"))
...
if(0){
  ...
  ggsave(filename = glue(work_directory, 'report/PCA_cell_cycle_after.pdf'))
}
#####

residual_matrix <- merge(meta_data, residual_matrix, by = 'ID')
saveRDS(residual_matrix, file = glue(work_directory, 'data/intermediate_data/residual_matrix.rds'))
