...
ggsave(file = glue(work_directory, 'report/violin_plot_QC.pdf'))
...
if(0){
  ...
  print(pca_plot)
  ggsave(file = glue(work_directory, 'report/PCA_cell_cycle_before.pdf'))
  pca_plot <- DimPlot(Cleary_sub, reduction = "pca", group.by = "10X_channel") + ggtitle("PCA colored by cell cycle")
  print(pca_plot)
  ggsave(file = glue(work_directory, 'report/PCA_chennel_before.pdf'))
}
#
...
saveRDS(residual_list, file = glue(work_directory, "data/intermediate_data/poisson_residual.rds"))
