...
saveRDS(CSCORE_result, file = glue::glue(work_directory, 'data/CSCORE_result_', gene_count, '_gene.rds'))
CSCORE_result <- readRDS(file = glue::glue(work_directory, 'data/CSCORE_result_', gene_count, '_gene.rds'))
...
module_list
saveRDS(module_list, file = glue::glue(work_directory, 'data/module_list_', gene_count,'_genes.rds'))

# module_list <- readRDS(glue("{work_directory}data/module_list_{gene_count}_genes.rds"))
##this first pass is based on the correlation
##the cluster 1 corresponds to the gray cluster, the genes that are not asscoiated with others
...
saveRDS(ego_result, file = glue::glue(work_directory, 'data/ego_result_', gene_count,'_genes.rds'))
...
saveRDS(finer_module_list, file = glue::glue(work_directory, 'data/finer_module_list_', gene_count,'_genes.rds'))

finer_module_list <- readRDS(file = glue::glue(work_directory, 'data/finer_module_list_', gene_count,'_genes.rds'))
...
write.csv(module_list_df, glue::glue(work_directory, 'data/module_list_df_', gene_count,'_genes.csv'), row.names = FALSE)
...
saveRDS(ego_result, file = glue::glue(work_directory, 'data/ego_result_', gene_count,'_genes_second.rds'))
...
sink(glue::glue(work_directory, 'data/top_ego_result_', gene_count,'_genes.txt'))
for(cluster_index in 1:length(ego_result)){
  print(cluster_index)
  cat('\n')
...
ego_result <- readRDS(file = glue::glue(work_directory, 'data/ego_result_', gene_count,'_genes.rds'))
all_ego <- data.frame()
for(cluster_index in 1:length(ego_result)){
  one_ego <- ego_result[[cluster_index]]
  if(is.null(one_ego)) next
