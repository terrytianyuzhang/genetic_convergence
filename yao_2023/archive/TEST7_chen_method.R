...
dir.create(glue(work_directory, 'data/intermediate_data/9A1_chen_method/'), recursive = TRUE, showWarnings = FALSE)
...
gene_compared <- clustering[cluster_index == 1, ]$gene_name

control_one_module <- as.matrix(control[, ..gene_compared])
treatment_one_module <- as.matrix(treatment[, ..gene_compared])

test_result <- apval_Chen2010(control_one_module, 
                              treatment_one_module, eq.cov = FALSE)

output_filename <- paste0(work_directory, 'data/intermediate_data/9A1_chen_method/', treatment_name, '_module_', module_index,'.rds')
