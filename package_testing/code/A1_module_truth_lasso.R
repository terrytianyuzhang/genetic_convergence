...
# ----------------- Examples -----------------
# A) Exactly your request: modules of size 30; tweak per-module averages easily.
#    Say we want tr1 to have +2 in module 1 and +1 in module 4;
#    tr2 to have +3 in module 3; control stays at 0 everywhere.
n_samples  <- 200
n_modules  <- 7
module_sz  <- 5

mu_control_mods <- c(0, 0, 0, 0, 0, 0, 0)   # length n_modules
mu_tr1_mods <- c(0, 2, 0, 2, 0, 0, 0)   # length n_modules
mu_tr2_mods <- c(2, 0, 0.5, 0, 0, 0, 0)   # length n_modules
...
sim <- simulate_modular_data(
  ...
  sd_within   = c(2,1,1,1,1,1,1)                # homoskedastic by default
)

# B) Heteroskedasticity by module (optional):
#    e.g., modules 1..7 have SDs 1,1,1.2,1,1.5,1,1
# sim2 <- simulate_modular_data(
#   n_samples   = n_samples,
#   n_modules   = n_modules,
#   module_size = module_sz,
#   mu_tr1      = mu_tr1_mods,
#   mu_tr2      = mu_tr2_mods,
#   sd_within   = c(1,1,1.2,1,1.5,1,1)
# )

# C) If you want full custom covariance (e.g., correlations) for a group, pass Sigma_* explicitly.
...
for (i in 1:n_simulations) {
  ...
  result <- convergence_testing(sim$control, sim$treatment1, sim$treatment2,
                                lambda_type = 'lambda.1se', n_folds = n_folds)
...
visualize_convergence_top_genes(
  fold_data = result$fold_data[1:5],
  top_n = 30
)
