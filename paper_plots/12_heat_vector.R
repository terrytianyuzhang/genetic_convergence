...
# Output directory
output_dir <- "~/Documents/genetic_convergence/paper_plots/data/"
...
for (plot_index in 1:length(my_plot)) {
  ...
  p <- ggplot(df, aes(x = x, y = y, fill = fill_color)) +
    geom_tile() +
    scale_fill_identity() +
    theme_void() +
    coord_fixed(ratio = 1)
  ...
  # Save using ggsave
  ggsave(
    filename = file.path(output_dir, paste0("heat_vector_", plot_index, "_", group, ".pdf")),
    plot = p,
    width = 5,
    height = 1,
    units = "in"
